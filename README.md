...
1. **ファイルを選択** — PubMed からダウンロードした文献リスト `.txt` / `.nbib` ファイルをドラッグ＆ドロップまたは選択する
2. **出力フィールドを選択** — デフォルトで主項目にチェックが入っている。任意で変更可能
3. **変換＆ダウンロード** — Excel(.xlsx) または CSV(.csv) ボタンをクリックする


**デモ版はこちら(実利用時は自分のPC上で使って下さい)↓** : [動作確認用SAMPLEデータはこちら](sample/sample_pubmed.txt) 
[![スクリーンショット](img/2026-03-24_001_00_mainScreenshot.png)](https://hellboy84.github.io/PubMed2ExcelCnoverter/pubmed2excel.html)
